...
**notification_events** (outbox) — `user_id` (recipient), `event`, `params jsonb`, `booking_id`, `channels text[]`, `created_at`, `processed_at`, `attempts`, `last_error`, `locked_until` (T12: held by the dispatcher, or waiting before a retry). Written by the RPC functions in the same transaction as the change.
...
- Trial warnings and trial end arrive with T14.
