...
**client_reminders** (T19d) — `(booking_id, kind in ('review','service'))` primary key, `sent_at`: the review request and the service reminder already sent for a finished job, so each goes once. RLS on, no grants (job functions only). `profiles.service_reminders boolean default true` — the client's switch in Cont (column grant to `authenticated`). `profiles.review_requests` and `profiles.service_reminders` (both `default true`, browser-updatable) turn each off (after T19d, `schema_version` 30); the review card on Caută follows `review_requests`. Neither job writes to a suspended account; a service switched off in the catalog is not reminded. The `service_due` tap carries `&masina=<car>`, which picks that garage car on step 4.
...
- **Subscription:** one Stripe product, monthly recurring price in RON; for 3, 6 or 12 months the same product as `price_data` with `recurring: { interval: month, interval_count: N }` at the period's price, and the Stripe subscription's metadata carries `billing_months` and `discount_percent` (the owner picks the period on Abonament; `stripe-checkout` takes `months`, refuses another value with `period_invalid`). The colleagues' item follows the same period and discount; on a period longer than a month a seat change is `always_invoice` (charged at once) instead of `create_prorations`. Checkout (hosted) to start, Customer Portal to change card / cancel / see invoices. Trial is tracked by us (`subscriptions.trial_ends_at`, 90 days from sign-up); a card given during the trial is only saved and the first charge is at the end of the trial (Stripe `trialing`); after the trial the first charge is at checkout. Webhook events: `checkout.session.completed`, `customer.subscription.created/updated/deleted`, `invoice.paid`, `invoice.payment_failed`. Final failed retry or trial end without payment → `status = inactive`, `shops.active = false`. Payment → `active`, `shops.active = true` immediately.
...
**Type:** one typeface, **Inter** (variable, bundled with the app: `@fontsource-variable/inter`, the browser loads only the Latin + Latin Extended files, ~130 KB once), system stack as fallback. Headings bold 700 with slightly tight letter-spacing; buttons bold; labels semibold. **Sentence case everywhere** — no forced capitals, except the wordmark (Inter 800, uppercase). Plates, times, phones, prices, km use the same font with even-width digits: `.mono` / `--font-mono` = `'Inter Figures'` (only Inter's digits 0–9 in their even-width forms, ~4 KB, inlined in the CSS; `src/styles/inter-figures.woff2`, made by `scripts/gen-fonts.py`) in front of Inter. Never `tabular-nums`: in Inter it also widens hyphens, dots, colons and spaces ("C‑00001" looked typewritten). No italics (Inter is loaded upright only). The tokens `--font-display` and `--font-mono` remain as names for the roles. The history report PDF uses the same Inter (Regular / Bold, digits even-width) and sentence-case labels. (Changed after T19d at Eduard's request; overrides the reference demo's Arial Narrow uppercase.)
