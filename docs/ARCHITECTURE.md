...
- **Browser** (`src/lib/monitoring.ts`, DSN from the public `VITE_SENTRY_DSN`, environment = Netlify context, release = `COMMIT_REF`): uncaught errors and unhandled rejections (`installGlobalHandlers`, not browser noise or extension stacks); errors while drawing a screen (the React root's `onCaughtError` / `onUncaughtError` in `src/main.tsx`; a chunk that cannot load only as a warning when online); RPC and table answers that are neither a business code nor a lost connection (`failure()` in `src/data/rpc.ts`, tagged `rpc: <function>`); the schema mismatch in production. At most 10 reports per page load, each distinct error once; the last 20 screens visited go along as breadcrumbs.
