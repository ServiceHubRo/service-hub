...
- **The PDF:** A4, white, the look of `docs/reference-report.pdf`: header (mark + wordmark, title, generated date and time in Bucharest, the site), the car (plate, year, VIN), the code box, the jobs table (date, shop + city, service + accepted lines + work, **km**, cost; header row repeated on new pages), total, jobs / period / "KM la ultima lucrare", the odometer note, the verification box (`<APP_URL host>/verifica` + code — `service-hub.ro` once APP_URL is set in T19), the disclaimer, "Pagina n din m". In the language chosen at checkout; text the fonts cannot print (emoji, other scripts) is left out.
