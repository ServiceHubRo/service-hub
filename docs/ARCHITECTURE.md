...
- Browser may update only `name`, `phone`, `lang`, `*_prompt_dismissed_at` (column grants), and since T19d `review_requests`, `service_reminders` (Cont → Remindere, both `default true`).
...
- `interval_months int` (T19d, 1–120, null = none): months between two jobs of that service, for the client's service reminder (§10). Set by the admin in Catalog through `admin_set_service_interval(id, request_id, months?)`; the migration set oil / full service / cabin filter / A/C cleaning / winter prep / LPG service / mobile oil change 12, air and fuel filter / brake fluid / A/C recharge 24, spark plugs / coolant 48, timing belt 60.
...
- `status` — see §3
- Timestamps: `status_changed_at`, `confirmed_at`, `inspection_started_at`, `started_at`, `done_at`, `cancelled_at`, `reminder_sent_at`; T19d: `review_requested_at` (the review request went), `service_reminded_at` (the service reminder counted from this job went)
...
The event → recipient matrix is FR §7. Event codes: `booking_requested, booking_confirmed, booking_declined, booking_rescheduled, booking_cancelled_client, booking_cancelled_shop, booking_cancelled_admin, inspection_started, quote_sent, quote_replaced, quote_withdrawn, quote_accepted, quote_partially_accepted, quote_refused, quote_expiring, quote_expired, work_started, job_done, no_show, appointment_reminder, doc_expiry, new_message, new_review, review_reply, review_report_decided, daily_digest, trial_ending, payment_failed, shop_inactive, invoice_paid, report_ready, broadcast`; T19d: `review_request, service_due` (client, push only, low urgency).
...
- **Text keys** (`notification_texts` overrides use the same): `<side>.<event>` plus variants — `client.booking_declined_reason`, `shop.quote_refused_fee`, `client.appointment_reminder_today`, `client.doc_expiry_today` / `_past`. Placeholders: `{shop} {client} {sender} {preview} {car} {car_plate} {service} {ref} {when} {slot} {reason} {total} {total_sent} {cost} {fee} {deadline} {deadline_rel} {rating} {doc} {days} {expiry} {digest} {code}`; T19d: `{service_lower}` (the service name inside a sentence: first letter small unless it starts with an acronym) and `{last}` (month and year of the last job). T15: `report_ready` (push + email to the client: the car, the code, a tap opens `/c/cont/rapoarte`).
- **Taps:** client → `/c/programari?p=<booking>` (the card is scrolled to and outlined; T19d `review_request` adds `&recenzie=1`: the review form is open), `service_due` → `/c/service/<shop>/programare?serviciu=<service>&masina=<car>&pas=2` (the same service at the same shop, that garage car picked on step 4; a shop no longer bookable shows its notice, a service no longer offered falls back to step 1), `/c/mesaje/<thread>`, `/c/garaj/<car>`; shop → `/s/programari?tab=cereri&p=` for requests, `/s/programari?p=` for active bookings, `/s/istoric?q=<ref>` for ended ones, `/s/mesaje/<thread>`, `/s/cont/recenzii`, `/s/panou` (digest). The service worker (`public/sw.js`, caches nothing) focuses an open window and posts the path to the app (`PushBridge` routes it without a reload), or opens a new window.
...
| daily 10:00 Bucharest | review requests for yesterday's jobs; service reminders ~2 weeks before due (T19d) |
...
- **T19d** (both from `run_hourly_jobs` at 10:xx Bucharest, push only): `send_review_requests(now)` — bookings `done` on one of the last three Bucharest days before today (a missed run is caught the next day), no review, the review window still open, the client's `review_requests` on, not suspended or deleted; once (`review_requested_at`); event `review_request`. `send_service_reminders(today)` — for each garage car (`bookings.car_id`, still owned by the client) and each enabled service with `interval_months`, the last `done` job of that service on that car; when its due day (job day in Bucharest + interval) is between today and 14 days from now, one `service_due` (params: the car's current garage details, `last_date`, `due_date`, `months`, plus the booking params), recorded on that job (`service_reminded_at`), so a newer job starts over; not when the car has an active booking for that service or the client turned `service_reminders` off. The review card on Caută (`ReviewPromptCard`) is the app's own: the latest finished job whose review can still be left, from the bookings already loaded, hidden when `review_requests` is off.
