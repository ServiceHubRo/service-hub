...
- Database migrations and Edge Functions reach Supabase automatically: the GitHub Action **"Deploy Supabase"** runs on every push to a pull request and on every merge to `main` (set up in T02, using secrets stored in GitHub, which you never see), so the preview link always has the database it needs. Since T19a pull requests go to a separate **test** project and `main` to the **real** one (until Eduard creates the test project, both still go to the real one — `docs/LANSARE.md`). Eduard only runs one-off SQL you hand him (for example `promote_to_admin`) and changes dashboard settings you describe step by step.
...
10. **Secrets:** the frontend only ever has public values: `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` (the public "anon"/"publishable" key), plus the optional `VITE_TURNSTILE_SITE_KEY` and `VITE_SENTRY_DSN`, both public by design. The service role key, Stripe secret, webhook secrets, VAPID private key, Resend and SMSO keys exist only as Supabase Edge Function secrets. Nothing secret is committed. `.env` is git-ignored; `.env.example` lists names only.
